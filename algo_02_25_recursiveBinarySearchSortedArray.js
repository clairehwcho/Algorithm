/*
Recursive Binary Search
Input: SORTED array of ints, int value
Output: bool representing if value is found
Recursively search to find if the value exists, do not loop over every element.
Approach:
Take the middle item and compare it to the given value.
Based on that comparison, narrow your search to a particular section of the array
*/


const nums1 = [1, 3, 5, 6];
const searchNum1 = 4;
const expected1 = false;

const nums2 = [4, 5, 6, 8, 12];
const searchNum2 = 5;
const expected2 = true;

const nums3 = [3, 4, 6, 8, 12];
const searchNum3 = 3;
const expected3 = true;

const nums4 = [3, 4, 6, 8, 12, 20];
const searchNum4 = 5;
const expected4 = false;
/**
 * Add params if needed for recursion
 * Recursively performs a binary search (divide and conquer) to determine if
 * the given sorted nums array contains the given number to search for.
 * - Time: O(?).
 * - Space: O(?).
 * @param {Array<number>} sortedNums
 * @param {number} searchNum
 * @returns {boolean} Whether the searchNum was found in the sortedNums array.
 */
function recursiveBinarySearch (sortedNums, searchNum) {
    let startIdx = 0;
    let midIdx = Math.floor((sortedNums.length) / 2);
    let endIdx = sortedNums.length - 1;

    if (sortedNums[midIdx] === searchNum) {
        return true;
    }
    else if (sortedNums[midIdx] > searchNum) {
        return recursiveBinarySearch(sortedNums.slice(startIdx, midIdx), searchNum);
    }
    else if (sortedNums[midIdx] < searchNum) {
        return recursiveBinarySearch(sortedNums.slice(midIdx + 1, endIdx + 1), searchNum);
    }
    else {
        return false;
    }
};

console.log(recursiveBinarySearch(nums1, searchNum1));
console.log(recursiveBinarySearch(nums2, searchNum2));
console.log(recursiveBinarySearch(nums3, searchNum3));
console.log(recursiveBinarySearch(nums4, searchNum4));